...
module Zaniah
  Theme = Data.define(:name, :appearance, :colors, :spacing, :radii, :shadows, :typography, :motion) do
    class << self
      ...
      def build(name, appearance, colors)
        new(name, appearance, const_get(:Colors).new(*colors.map { |color| Color.parse(color) }),
          ...
          const_get(:Typography).new("sans-serif", "monospace", 11, 13, 14, 16, 20, 24, 400, 500, 600, 700, 1.2, 1.5, 1.75),
          const_get(:Motion).new(0.12, 0.18, 0.28, :ease_in_out, :ease_out, :ease_in, false))
      end
    end
  end

  Theme.const_set(:Colors, Data.define(:background, :surface, :surface_hover, :surface_pressed,
  ...
  Theme.const_set(:Typography, Data.define(:font_sans, :font_mono, :size_xs, :size_sm, :size_md,
